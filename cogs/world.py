...
class World:
    def __init__(
        ...
        if passes == None: self.passes = ((size[0] + size[1])/2)-1
        else: self.passes = passes
        ...
        for y in range(size[1]):
            for x in range(size[0]):
                if math.sqrt((x-(size[0]/2))**2 + (y-(size[1]/2))**2) > size[0]-((3/5)*min(size[0], size[1])): n = Biome(t=Biomes.OCEAN, coords=(x, y))
                else: 
            ...
            for y in range(size[1]):
                for x in range(size[0]):
                    if (min(x, y) == 0) or (x == len(self.map[0])-1) or (y == len(self.map)-1): continue
                        ...
                        ne = []
                        ne.append(self.map[y-1][x])
                        ne.append(self.map[y+1][x])
                        ne.append(self.map[y][x-1])
                        ne.append(self.map[y][x+1])
